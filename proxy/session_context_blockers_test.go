...
func TestSessionContextBlockersReachClientUsageAndServiceErrors(test *testing.T) {
	...
	require.Contains(test, failure.Message, "input[0].encrypted_content")
	require.Contains(test, failure.Message, "reasoning")
	require.NotContains(test, failure.Message, "private-value")
	details, err := json.Marshal(failure.Details)
	require.NoError(test, err)
	require.Equal(test, "encrypted_content", gjson.GetBytes(details, "context_blockers.0.kind").String())
	require.Len(test, usageRequestDiagnosticState(request).AccountFailover.ContextBlockers, 1)
	api.SendError(request, failure)
	finish()
	page := serviceErrorTestPage(test, handler)
	require.Len(test, page.Items, 1)
	require.Len(test, page.Items[0].AccountFailover.ContextBlockers, 1)
	require.Equal(test, "input[0].encrypted_content", page.Items[0].AccountFailover.ContextBlockers[0].Path)
}
