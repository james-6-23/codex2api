...
func (h *Handler) buildAccountResponse(
	...
	sessionCapacityReserved := int64(0)
	...
	if !isOpenAIResponsesAccount && !isGrokAccount && !isAntigravityAccount && !isClaudeAccount {
		...
		if configured, ok := row.GetCredentialInt64(auth.SessionCapacityReservedCredentialKey); ok {
			sessionCapacityReserved = max(0, min(sessionCapacityMax, configured))
		}
		...
		if sessionCapacityEnabled && runtimeAccount != nil {
			sessionCapacityCurrent, sessionCapacityReservedCurrent = h.store.AccountSessionSlotCounts(row.ID, time.Now())
		}
	}
	...
	resp := accountResponse{
		...
		SessionCapacityReservedCurrent: sessionCapacityReservedCurrent,
