...
type DialOptions struct {
	...
	// CompressionLevel selects 1..9 for the 32 KiB encoder. Zero defaults to 1.
	// Smaller negotiated windows use the custom-window encoder's fixed strategy.
	CompressionLevel int

	// DisableFragmentation sends each Write message in a single data frame.
	// By default, outgoing data frames are bounded to 16 KiB.
...
func dial(ctx context.Context, urls string, opts *DialOptions, rand io.Reader) (_ *Conn, _ *http.Response, err error) {
	...
	if opts.CompressionLevel < 1 || opts.CompressionLevel > 9 {
		return nil, nil, fmt.Errorf("compression level must be between 1 and 9")
	}
	...
	if opts.CompressionMode != CompressionDisabled {
		...
		copts.clientMaxWindowBitsOffered = true
	}
	...
	return newConn(connConfig{
		...
		disableFragmentation: opts.DisableFragmentation,
...
func verifyServerExtensions(copts *compressionOptions, h http.Header) (*compressionOptions, error) {
	...
	_copts := *copts
	copts = &_copts
	offeredWindowBits := copts.clientMaxWindowBitsOffered
	copts.clientMaxWindowBitsOffered = false

	seen := make(map[string]bool, len(ext.params))
	for _, p := range ext.params {
		name, value, hasValue := strings.Cut(p, "=")
		name, value = strings.TrimSpace(name), strings.TrimSpace(value)
		if seen[name] {
			return nil, fmt.Errorf("duplicate permessage-deflate parameter: %q", name)
		}
		seen[name] = true
		if name == "client_max_window_bits" || name == "server_max_window_bits" {
			bits, valid := parseCompressionWindowBits(value)
			if !hasValue || !valid {
				return nil, fmt.Errorf("invalid permessage-deflate window parameter: %q", p)
			}
			if name == "client_max_window_bits" {
				if !offeredWindowBits {
					return nil, fmt.Errorf("server returned unoffered client_max_window_bits")
				}
				copts.clientMaxWindowBits = bits
			}
			// The 32 KiB decoder can also decode a smaller server window.
			continue
		}
		switch p {
