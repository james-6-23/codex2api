...
const (
	...
	// CompressionContextTakeover compresses each message greater than 128 bytes reusing up to a 32 KB sliding window from
	...
	// A negotiated client_max_window_bits value can reduce the client's encoding
	// window; the encoder implementation and memory cost then depend on that value.
	//
...
func (copts *compressionOptions) String() string {
	...
	} else if copts.clientMaxWindowBits != 0 {
...
// Reset retains window and level settings, so each combination has its own pool.
var flateWriterPools [16][10]sync.Pool

func getFlateWriter(w io.Writer, windowBits, level int) *pooledFlateWriter {
	if windowBits == 0 {
		windowBits = 15
	}
	if level < 1 || level > 9 {
	...
	if windowBits < 15 {
		// The custom-window API has no level option; avoid redundant pools.
		level = 0
	}
	if fw, ok := flateWriterPools[windowBits][level].Get().(*pooledFlateWriter); ok {
		fw.Reset(w)
		return fw
	}
	var fw compressionWriter
	if windowBits == 15 {
		fw, _ = flate.NewWriter(w, level)
	} else {
		// Negotiation restricts this value to 9..14, all supported by the encoder.
		fw, _ = windowflate.NewWriterWindow(w, 1<<windowBits)
	}
	return &pooledFlateWriter{compressionWriter: fw, windowBits: windowBits, level: level}
}

func putFlateWriter(w *pooledFlateWriter) {
	flateWriterPools[w.windowBits][w.level].Put(w)
}
