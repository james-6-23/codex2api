...
func (c *Conn) handleControl(ctx context.Context, h header) (err error) {
	...
	case opPing:
		...
		return c.writeControl(context.Background(), opPong, b)
	case opPong:
	...
	// The outer inflater may still be using its dictionary and pooled reader.
	// Leave readMu held until that read unwinds before cleanup can reclaim them.
	go func() {
		if !closeSent {
