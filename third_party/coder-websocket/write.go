...
//
// Data frames are bounded so control frames can interleave with large messages.
...
func (mw *msgWriter) ensureFlate() {
	...
	if mw.flateWriter == nil {
		windowBits := 15
		if mw.c.client && mw.c.copts.clientMaxWindowBits != 0 {
			windowBits = mw.c.copts.clientMaxWindowBits
		}
		mw.flateWriter = getFlateWriter(mw.trimWriter, windowBits, mw.c.compressionLevel)
	}
	mw.flate = true
}
...
func (c *Conn) write(ctx context.Context, typ MessageType, p []byte) (int, error) {
	...
	if !c.flate() || len(p) < c.flateThreshold {
		return c.writeDataFrames(ctx, true, false, c.msgWriter.opcode, p)
	}
...
// writeCompressedFrame compresses p once and writes bounded data frames.
func (mw *msgWriter) writeCompressedFrame(ctx context.Context, p []byte) (int, error) {
	...
	// Compress once, then split the wire output into bounded data frames.
	...
	_, err = mw.c.writeDataFrames(ctx, true, true, mw.opcode, buf.Bytes())
	if err != nil {
		return 0, err
	}
	return len(p), nil
...
func (mw *msgWriter) write(p []byte) (int, error) {
	n, err := mw.c.writeDataFrames(mw.ctx, false, mw.flate, mw.opcode, p)
	if err != nil {
